...
class VoyagerSettingsController extends Controller
{
    ...
    public function index_group($group)
    {
        ...
        $group_alias = $group;
        $group_title = Str::ucfirst($group_alias);
        ...
        return Voyager::view('voyager::settings.single_group', compact('group_alias', 'group_title', 'data', 'settings', 'groups', 'active'));
    }

    public function update_group($group, Request $request)
    {
        // Check permission
        $this->authorize('edit', Voyager::model('Setting'));
