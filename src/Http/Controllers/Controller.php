...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        foreach ($rows as $row) {
            ...
            if (in_array($row->type, ['multiple_images_with_sort', 'file']) && !is_null($content)) {
                $new_content = $content;
                $old_content = $request->{$row->field.'_ext'};

                if (empty($old_content)) {
                    $new_content = collect($new_content);
                    $new_content = $new_content->sortBy('sort');
                    $content = json_encode($new_content);
                } else {
                    $new_content = collect(array_merge($old_content, $new_content));
                    $new_content = $new_content->sortBy('sort');
                    $content = json_encode($new_content);
                }
            }
    ...
    public function getContentBasedOnType(Request $request, $slug, $row, $options = null)
    {
        switch ($row->type) {
            ...
            /********** MULTIPLE IMAGES WITH SORT TYPE **********/
            case 'multiple_images_with_sort':

                $files = [];
                if ($request->file($row->field)){
                    $pathes = (new MultipleImage($request, $slug, $row, $options))->handle();
